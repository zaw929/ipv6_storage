...
function showAccountView(folderView) {
	...
	if (folderView.account != null) {
		...
		$("#tb")
				.append(
						"<button class='btn btn-link rightbtn hidden-xs' data-toggle='modal' style='background-color:rgba(23,186,194,1);color: #fff;border-radius: 4px' data-target='#logoutModal'>注销 ["
								+ folderView.account
	...
	if (authList != null) {
		...
		if (folderView.enableDownloadZip && checkAuth(authList, "L")) {
			$("#packageDownloadBox")
					.html(
							"<button class='btn btn-link navbar-btn' onclick='showDownloadAllCheckedModel()' style='color:#000'><span class='glyphicon glyphicon-briefcase'></span> 打包下载</button>");
		}else{
...
function showRenameFolderModel(folderId, folderName, type) {
	$("#renameFolderBox").html(
			"<button type='button' class='btn btn-primary' style='background-color: rgba(23,186,194,1);border: 0' onclick='renameFolder("
					+ '"' + folderId + '"' + ")'>修改</button>");
...
function showDownloadModel(fileId, fileName) {
	...
	$("#downloadFileBox")
			.html(
					"<button id='dlmbutton' type='button' class='btn btn-primary' style='border: 0;background-color: rgba(23,186,194,1);' onclick='dodownload("
							+ '"' + fileId + '"' + ")'>开始下载</button>");
